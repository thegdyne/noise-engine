# How to Troubleshoot and Fix Pack Issues
...
## Issue: RENDER_FAILED (timeout)

**Symptom:** Generator hangs during NRT render (30s timeout), no SC error output.

### Cause 1: Unary Minus Before Variable
...
sed -i '' 's/-contrast/contrast.neg/g' packs/my_pack/generators/my_gen.scd
```

### Cause 2: Unary Minus Before Parenthesis
...
### Cause 4: Variable Name Collision with `gate`
...
### Cause 5: Complex Buffer Patterns
...
**Symptom:** RMS below -40 dB (or -55 dB for impulsive).

### Cause 1: Missing Triggers for Percussive Generator
...
### Cause 2: Wrong midi_retrig Setting
...
### Cause 3: Squelch/Gate Logic at Test Defaults

Generator has gating logic that closes at the 0.5 test defaults.

**Diagnose:** Check the DSP for threshold comparisons:
```bash
...
**Symptom:** Active frames below 30% (or 5% for impulsive).

### Cause: Percussive Generator Tested in Drone Mode
...
**Symptom:** Peak at or above 0.0 dB.
...
**Symptom:** Mean signal significantly above 0.
...
**Symptom:** Level grows >6 dB over render duration.
...
**Fix:**
```bash
# Add pack_id derived from directory name
python3 -c "
import json
from pathlib import Path
...
### Reserved pack_id (e.g., "core")

Legacy packs with reserved names fail Forge validation. The gate auto-skips legacy packs (no `forge_` prefix in SynthDefs).

### Undeclared Variable

**Diagnose:** Check error message for variable name.

**Fix:** Add to `var` declaration at top of SynthDef:
```bash
...
## Issue: UTF-8 / ASCII Corruption

**Symptom:** Mojibake characters, syntax errors with `??` or similar.
...
## Workflow: Debug a Specific Generator
```bash
PACK=my_pack
GEN=my_generator
...
## Known Issue Patterns to Accept
