...
def run_sc_verification(repo: Path) -> tuple[bool, str]:
    ...
    for name in ["imaginarium_verify_contract_v6.scd", "imaginarium_verify_contract_v5.scd", "imaginarium_verify_contract.scd"]:
        sc_script = repo / "scripts" / name
        if sc_script.exists():
            break
    else:
        return False, f"Missing verify script in scripts/ (tried v6, v5, canonical)"
